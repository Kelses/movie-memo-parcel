...
async function getPopularMovies() {
    const response = await fetch(`${(0, _networkJs.apiConfig).tmdbBaseUrl}/movie/popular?api_key=${(0, _networkJs.apiConfig).tmdbApiKey}`);
    ...
    try {
        if (!input) {
            ...
            movies.forEach((movie)=>{
                ...
                addButton.style.backgroundColor = movieJournal.find((fav)=>fav.id === movie.id) ? "#28A745" : "bg-yellow-400";
            });
        } else {
            // Filter movies based on input
            const filteredMovies = movies.filter((movie)=>movie.title.toLowerCase().includes(input.toLowerCase()));
            movieContainer.innerHTML = "";
            if (!filteredMovies.length) movieContainer.innerHTML = `<p class="text-white">No movies found matching "${input}"</p>`;
...
function createMovieCard(movie) {
    ...
    movieCard.innerHTML = `
    <img
      src="${(0, _networkJs.apiConfig).imageBaseUrl + movie.poster_path}"
      alt="${movie.title}"
  ...
  `;
    ...
    addButton.addEventListener("click", ()=>{
        (0, _storageJs.addToJournal)(movie);
        favorites.push(movie);
        saveFavorites(movie.id);
        // Change button color if movie is in the journal
        const btn = document.getElementById(`add-button-${movie.id}`);
        if (isInTheList(movie)) btn.style.backgroundColor = "#28a745";
...
},{"./network.js":"6ZlL7","./storage.js":"bkDau"}],"6ZlL7":[function(require,module,exports,__globalThis) {
// API configuration object containing the API key, base URL, and image base URL
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "apiConfig", ()=>apiConfig);
...
},{}],"bkDau":[function(require,module,exports,__globalThis) {
// Function to add a product to the cart
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "addToCart", ()=>addToCart);
parcelHelpers.export(exports, "addToJournal", ()=>addToJournal);
parcelHelpers.export(exports, "removeCard", ()=>removeCard);
